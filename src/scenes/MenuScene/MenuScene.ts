...
export class MenuScene extends BaseScene {
    ...
    private getTitle(): PIXI.Text {
        ...
        const title = new PIXI.Text('Softgames Demos', {
            fill:       0xffffff,
            fontSize:   titleSize,
            fontWeight: 'bold',
            fontFamily: 'Arial',
            dropShadow: true,
            dropShadowColor: 0x4466ff,
            dropShadowDistance: rem(4),
            dropShadowBlur: rem(12),
        } as any);
    ...
    private buildUI(): void {
        ...
        (bg as any).rect(0, 0, width, height).fill({ color: 0x0a0a1a });
        this.addChild(bg);
    ...
    private buildCard(
        ...
        const drawBg = (hover: boolean) => {
            bg.clear();
            (bg as any)
                .roundRect(0, 0, w, h, rem(14))
                .fill({ color: hover ? color : 0x16162a, alpha: hover ? 0.25 : 0.9 });
            (bg as any)
                .roundRect(0, 0, w, h, rem(14))
                .stroke({ color, alpha: hover ? 1 : 0.45, width: hover ? 2 : 1.5 });
        };
        drawBg(false);
        card.addChild(bg);

        const glow = new PIXI.Graphics();
        (glow as any).roundRect(-rem(6), -rem(6), w + rem(12), h + rem(12), rem(20))
            .fill({ color, alpha: 0.12 });
        glow.visible = false;
        card.addChildAt(glow, 0);

        const emojiText = new PIXI.Text(emoji, { fontSize: rem(42) } as any);
