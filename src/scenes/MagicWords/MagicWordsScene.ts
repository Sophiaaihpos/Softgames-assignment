...
import { ACCENT_COLORS, BASE_AVATAR_SIZE, BASE_EMOJI_SIZE, BASE_FONT_SIZE, BASE_LINE_HEIGHT, BASE_PADDING, BUBBLE_COLORS, BUBBLE_RADIUS } from './MagicWordsConstants';
...
export class MagicWordsScene extends BaseScene {
    ...
    private readonly onKey   = (e: KeyboardEvent) => {
        if (e.code === 'Space') { e.preventDefault(); this.renderNextLine(); }
    };
    ...
    stop(): void {
        ...
        (this.app.view as HTMLCanvasElement).removeEventListener('pointerdown', this.onClick);
    ...
    private async loadData(): Promise<void> {
        ...
        await Promise.allSettled([
            ...this.logic.data.emojies.map(e => this.cacheTexture(`emoji:${e.name}`, e.url)),
            ...this.logic.data.avatars.map(a => this.cacheTexture(`avatar:${a.name}`, a.url)),
        ]);
        this.showHint();
        this.renderNextLine();
    }
    ...
    private async cacheTexture(key: string, url: string): Promise<void> {
        ...
        try { this.textures.set(key, await this.loadTextureFromUrl(url)); }
        catch { console.warn(`Texture failed [${key}]`); }
    }
    ...
    private buildUI(): void {
        ...
        (bg as any).rect(0, 0, width, height).fill({ color: 0x080818 });
    ...
    private showHint(): void {
        const { isMobile } = getLayout(this.app.screen.width, this.app.screen.height);
        this.hint = new PIXI.Text(
            isMobile ? '👆 Tap to advance' : '␣ Space or click to advance',
            { fill: 0x6688aa, fontSize: this.fontSize - 2, fontFamily: 'Arial', fontStyle: 'italic' } as any,
        );
    ...
    private repositionHint(): void {
        const { width, height, rem } = getLayout(this.app.screen.width, this.app.screen.height);
    ...
    private buildRow(line: DialogueLine, avatar?: Avatar): PIXI.Container {
        ...
        const avatarTex = avatar ? this.textures.get(`avatar:${avatar.name}`) : undefined;
        const bubble   = this.buildBubble(line.text, isLeft);
        const nameLabel = new PIXI.Text(line.name, {
            fill: isLeft ? 0x66aaff : 0xcc88ff,
            fontSize: this.fontSize - 2, fontWeight: 'bold', fontFamily: 'Arial',
        } as any);
        ...
        if (isLeft) {
            ...
            const bubbleX = this.app.screen.width - this.padding - this.bubbleMaxWidth - avatarW - this.padding;
            nameLabel.position.set(bubbleX + this.bubbleMaxWidth - nameLabel.width, 0);
            bubble.position.set(bubbleX, nameH);
            avatarContainer.position.set(bubbleX + this.bubbleMaxWidth + this.padding, nameH);
        }
    ...
    private buildAvatar(tex: PIXI.Texture | undefined, isLeft: boolean): PIXI.Container {
        const container = new PIXI.Container();
        if (!tex) return container;

        const r      = this.avatarSize / 2;
        const mask   = new PIXI.Graphics();
        (mask as any).circle(r, r, r).fill({ color: 0xffffff });
        ...
        (ring as any).circle(r, r, r + 2)
            .stroke({ color: isLeft ? ACCENT_COLORS.left : ACCENT_COLORS.right, width: 2, alpha: 0.8 });
    ...
    private buildBubble(text: string, isLeft: boolean): PIXI.Container {
        ...
        const newLine  = () => { pen.x = this.padding; pen.y += this.lineHeight; };
        ...
        (bg as any).roundRect(0, 0, this.bubbleMaxWidth, totalH, BUBBLE_RADIUS).fill({ color });
        (bg as any).roundRect(0, 0, 4, totalH, BUBBLE_RADIUS).fill({ color: accent, alpha: 0.9 });
