...
export class PhoenixFlameScene extends BaseScene {
  ...
  private rebuildBackground(): void {
    ...
    bg.rect(0, 0, width, height).fill({ color: 0x0d0500 });

    for (let y = 0; y < height; y += rem(6)) {
      bg.rect(0, y, width, 1).fill({ color: 0x1a0800, alpha: 0.3 });
    }
    ...
    const glowDefs = [
      ...
      { r: height * 0.07, color: 0xff8800, alpha: 0.2 },
    ];
  ...
  private buildTextures(): void {
    const flameRings = [
      ...
      { r: 5, color: 0xffffff, alpha: 1.0 },
    ];
    const fg = new PIXI.Graphics();
    flameRings.forEach(({ r, color, alpha }) =>
      fg.circle(54, 54, r).fill({ color, alpha }),
    );
