...
export class PhoenixFlameLogic {
    ...
    private calcTint(progress: number): number {
        ...
        if (progress < 0.10) {
            const q = progress / 0.10;
            r = 255; g = 255; b = Math.round(255 * (1 - q));

            // Yellow orange (0.10–0.35)
        } else if (progress < 0.35) {
            const q = (progress - 0.10) / 0.25;
            r = 255; g = Math.round(255 - q * 175); b = 0;

            // Orange bright red (0.35–0.60)
        } else if (progress < 0.60) {
            const q = (progress - 0.35) / 0.25;
            r = 255; g = Math.round(80 - q * 80); b = 0;

            // Deep red dark crimson (0.60–0.85)
        } else if (progress < 0.85) {
            const q = (progress - 0.60) / 0.25;
            r = Math.round(255 - q * 120); g = 0; b = 0;
            
            // Crimson near-black ember (0.85–1.0)
        } else {
            const q = (progress - 0.85) / 0.15;
            r = Math.round(135 - q * 100); g = 0; b = 0;
        }
        return (r << 16) | (g << 8) | b;
    }
}