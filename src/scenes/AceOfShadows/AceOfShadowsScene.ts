...
export class AceOfShadowsScene extends BaseScene {
  ...
  private rebuildBackground(): void {
    ...
    (bg as PIXI.Graphics).rect(0, 0, width, height).fill({ color: 0x06120a });
    this.background = bg;
  ...
  private buildCardTextures(): void {
    this.cardTextures.forEach((t) => t.destroy());
    this.cardTextures = CARD_PALETTES.map(
  ...
  private buildCards(): void {
    ...
    for (let i = 0; i < CARD_COUNT; i++) {
      const sprite = new PIXI.Sprite(
        this.cardTextures[i % CARD_PALETTES.length],
      );
  ...
  private animateMove(cmd: MoveCommand): void {
    ...
    const globalTarget = targetStack.toGlobal(
      new PIXI.Point(0, cmd.targetOffsetY),
    );
