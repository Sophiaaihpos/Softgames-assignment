...
export function buildCardTexture(
    ...
    const roundRect = (x: number, y: number, rw: number, rh: number, radius: number) => {
        ctx.beginPath();
    ...
    ctx.beginPath();
    ctx.moveTo(r, 0); ctx.lineTo(w * 0.55, 0);
    ctx.lineTo(0, h * 0.45); ctx.lineTo(0, r);
    ctx.closePath();
    ctx.fillStyle = 'rgba(255,255,255,0.04)';
    ctx.fill();

    const cx = w / 2, cy = h / 2, pipR = w * 0.18;
    ctx.beginPath(); ctx.arc(cx, cy, pipR, 0, Math.PI * 2);
    ctx.fillStyle = pal.pip + '40'; ctx.fill();
    ctx.beginPath(); ctx.arc(cx, cy, pipR, 0, Math.PI * 2);
    ctx.strokeStyle = pal.pip + '8C'; ctx.lineWidth = 1; ctx.stroke();

    for (const [dx, dy] of [[8, 8], [w - 8, 8], [8, h - 8], [w - 8, h - 8]] as [number, number][]) {
        ctx.beginPath(); ctx.arc(dx, dy, 2, 0, Math.PI * 2);
        ctx.fillStyle = pal.accent + '80'; ctx.fill();
    }
