...
export class BackButton extends PIXI.Container {
    constructor(switchScene: (name: string) => void) {
        ...
        (bg as any).roundRect(0, 0, 130, 38, 10).fill({ color: 0x1a1a2e, alpha: 0.85 });
        (bg as any).roundRect(0, 0, 130, 38, 10).stroke({ color: 0xff4500, alpha: 0.8, width: 1.5 });

        const label = new PIXI.Text('← Menu', {
            fontFamily: 'Arial', fontSize: 16, fill: 0xff7733,
        } as any);
